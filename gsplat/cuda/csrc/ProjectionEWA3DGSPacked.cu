...
__global__ void frustum_culling_kernel(
    ...
    if (valid) {
        ...
        mat3 covar;
        // compute covar from quaternions and scales
        quats += col_idx * 4;
        scales += col_idx * 3;
...
template <typename scalar_t>
__global__ void frustum_culling_split_kernel(
    const uint32_t C,
    const uint32_t N,
    const scalar_t *__restrict__ means,    // [N, 3]
    const scalar_t *__restrict__ quats,    // [N, 4] Optional
    ...
    const float near_plane,
    const float far_plane,
    const int32_t split_point,     // NEW: x-coordinate of split point
    const bool left,               // NEW: true = process left side, false = process right side
    const int32_t
        *__restrict__ block_accum, // [C * blocks_per_row] packing helper
    // outputs
    int32_t *__restrict__ block_cnts,    // [C * blocks_per_row] packing helper
    ...
    if (valid) {
        ...
        // mask out gaussians outside the image region
        // Modified boundary check to consider split point
        int x_min = left ? 0 : split_point;
        int x_max = left ? split_point : image_width;
        
        if (mean2d.x + radius_x <= x_min || mean2d.x - radius_x >= x_max ||
            mean2d.y + radius_y <= 0 || mean2d.y - radius_y >= image_height) {
            valid = false;
        }
    }
...
template <typename scalar_t>
__global__ void projection_ewa_3dgs_packed_split_fwd_kernel(
    const uint32_t C,
    const uint32_t N,
    const scalar_t *__restrict__ means,    // [N, 3]
    const scalar_t *__restrict__ covars,   // [N, 6] Optional
...
void launch_frustum_culling_kernel(
    ...
    AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]() {
            frustum_culling_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
...
void launch_frustum_culling_split_kernel(
    // inputs
    const at::Tensor means,                // [N, 3]
    const at::optional<at::Tensor> quats,  // [N, 4] optional
    const at::optional<at::Tensor> scales, // [N, 3] optional
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means.scalar_type(),
        "frustum_culling_split_kernel",
        [&]() {
            frustum_culling_split_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
                   at::cuda::getCurrentCUDAStream()>>>(
...
void launch_projection_ewa_3dgs_packed_split_fwd_kernel(
    // inputs
    const at::Tensor means,                // [N, 3]
    const at::optional<at::Tensor> covars, // [N, 6] optional
    const at::optional<at::Tensor> quats,  // [N, 4] optional
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means.scalar_type(),
        "projection_ewa_3dgs_packed_split_fwd_kernel",
        [&]() {
            projection_ewa_3dgs_packed_split_fwd_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
                   at::cuda::getCurrentCUDAStream()>>>(
