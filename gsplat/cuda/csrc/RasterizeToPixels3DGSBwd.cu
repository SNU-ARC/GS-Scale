...
template <uint32_t CDIM, typename scalar_t>
__global__ void rasterize_to_pixels_3dgs_split_bwd_kernel(
    const uint32_t C,
    const uint32_t N,
    const uint32_t n_isects,
    const bool packed,
    ...
    // Calculate pixel ID based on split - same as forward kernel
    int32_t pix_id;
    if (left) {
    ...
    // this is the T AFTER the last gaussian in this pixel
    float T_final = inside ? (1.0f - render_alphas[pix_id]) : 1.0f;
    float T = T_final;
    // the contribution from gaussians behind the current one
    float buffer[CDIM] = {0.f};
    // index of last gaussian to contribute to this pixel
...
template <uint32_t CDIM>
void launch_rasterize_to_pixels_3dgs_split_bwd_kernel(
    // Gaussian parameters
    const at::Tensor means2d,                   // [C, N, 2] or [nnz, 2]
    const at::Tensor conics,                    // [C, N, 3] or [nnz, 3]
    const at::Tensor colors,                    // [C, N, 3] or [nnz, 3]
    ...
    // writes. This requires moving the channel padding from python to C side.
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dgs_split_bwd_kernel<CDIM, float>,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            shmem_size
        ) != cudaSuccess) {
        AT_ERROR(
...
#define __INS__(CDIM)                                                          \
    ...
    template void launch_rasterize_to_pixels_3dgs_split_bwd_kernel<CDIM>(      \
        const at::Tensor means2d,                                              \
        const at::Tensor conics,                                               \
        const at::Tensor colors,                                               \
        const at::Tensor opacities,                                            \
