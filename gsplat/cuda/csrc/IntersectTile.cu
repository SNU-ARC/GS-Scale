...
template <typename scalar_t>
__global__ void intersect_tile_split_kernel(
    const bool packed,
    const uint32_t C,
    const uint32_t N,
    const uint32_t nnz,
    ...
    // Calculate original tile boundaries
    uint2 tile_min, tile_max;
    tile_min.x = max(0, (uint32_t)floor(tile_x - tile_radius_x));
    tile_min.y = max(0, (uint32_t)floor(tile_y - tile_radius_y));
    tile_max.x = min((uint32_t)ceil(tile_x + tile_radius_x), tile_width);
    tile_max.y = min((uint32_t)ceil(tile_y + tile_radius_y), tile_height);

    // Convert split_point to tile boundary
    // split_point 320 with tile_size 16 -> tile boundary is 20
    // Left side: tiles 0-19, Right side: tiles 20-39
    uint32_t split_tile_boundary = split_point / tile_size;
    
    if (left) {
        tile_max.x = min(tile_max.x, split_tile_boundary);
    } else {
        tile_min.x = max(tile_min.x, split_tile_boundary);
    }

    // Check if we have any valid tiles after clamping
...
void launch_intersect_tile_split_kernel(
    // inputs
    const at::Tensor means2d,                    // [C, N, 2] or [nnz, 2]
    const at::Tensor radii,                      // [C, N, 2] or [nnz, 2]
    const at::Tensor depths,                     // [C, N] or [nnz]
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means2d.scalar_type(),
        "intersect_tile_split_kernel",
        [&]() {
            intersect_tile_split_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
                   at::cuda::getCurrentCUDAStream()>>>(
