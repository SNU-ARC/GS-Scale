...
std::tuple<at::Tensor, at::Tensor, at::Tensor> rasterize_to_pixels_3dgs_split_fwd(
    // Gaussian parameters
    const at::Tensor means2d,   // [C, N, 2] or [nnz, 2]
    const at::Tensor conics,    // [C, N, 3] or [nnz, 3]
    const at::Tensor colors,    // [C, N, channels] or [nnz, channels]
    ...
    uint32_t C = tile_offsets.size(0); // number of cameras
    uint32_t channels = colors.size(-1);
    at::Tensor renders, alphas, last_ids;
    if (left) {
        renders =
            at::empty({C, image_height, split_point, channels}, means2d.options());
        alphas =
            at::empty({C, image_height, split_point, 1}, means2d.options());
        last_ids = at::empty(
            {C, image_height, split_point}, means2d.options().dtype(at::kInt)
        );
    } else {
        renders =
            at::empty({C, image_height, image_width - split_point, channels}, means2d.options());
        alphas =
            at::empty({C, image_height, image_width - split_point, 1}, means2d.options());
        last_ids = at::empty(
            {C, image_height, image_width - split_point}, means2d.options().dtype(at::kInt)
        );
    }

#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_3dgs_split_fwd_kernel<N>(                         \
            means2d,                                                           \
            conics,                                                            \
            colors,                                                            \
            opacities,                                                         \
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor>
rasterize_to_pixels_3dgs_split_bwd(
    // Gaussian parameters
    const at::Tensor means2d,                   // [C, N, 2] or [nnz, 2]
    const at::Tensor conics,                    // [C, N, 3] or [nnz, 3]
    const at::Tensor colors,                    // [C, N, 3] or [nnz, 3]
...
#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_3dgs_split_bwd_kernel<N>(                   \
            means2d,                                                           \
            conics,                                                            \
            colors,                                                            \
            opacities,                                                         \
