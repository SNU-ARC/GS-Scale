...
template <uint32_t CDIM, typename scalar_t>
__global__ void rasterize_to_pixels_3dgs_split_fwd_kernel(
    const uint32_t C,
    const uint32_t N,
    const uint32_t n_isects,
    const bool packed,
    ...
    // Check if this pixel should be processed based on split
    bool should_process = true;
    if (left) {
        // Process left side: pixels with j < split_point
        should_process = (j < split_point);
    } else {
        // Process right side: pixels with j >= split_point
        should_process = (j >= split_point);
    }
    ...
    // Calculate local pixel coordinates for memory access
    int32_t pix_id;
    if (left) {
        pix_id = i * split_point + j;
    } else {
        pix_id = i * (image_width - split_point) + j - split_point;
    }
    
    // Use local coordinates for pixel ID

    // return if out of bounds or not in the processing region
    // keep not rasterizing threads around for reading data
    bool inside = (i < image_height && j < image_width) && should_process;
    bool done = !inside;
...
template <uint32_t CDIM>
void launch_rasterize_to_pixels_3dgs_split_fwd_kernel(
    // Gaussian parameters
    const at::Tensor means2d,   // [C, N, 2] or [nnz, 2]
    const at::Tensor conics,    // [C, N, 3] or [nnz, 3]
    const at::Tensor colors,    // [C, N, channels] or [nnz, channels]
    ...
    // writes. This requires moving the channel padding from python to C side.
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dgs_split_fwd_kernel<CDIM, float>,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            shmem_size
        ) != cudaSuccess) {
        AT_ERROR(
...
#define __INS__(CDIM)                                                          \
    ...
    template void launch_rasterize_to_pixels_3dgs_split_fwd_kernel<CDIM>(      \
        const at::Tensor means2d,                                              \
        const at::Tensor conics,                                               \
        const at::Tensor colors,                                               \
        const at::Tensor opacities,                                            \
