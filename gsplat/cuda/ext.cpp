...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("intersect_tile_split", &gsplat::intersect_tile_split);
    ...
    m.def(
        "projection_ewa_3dgs_packed_split_fwd",
        &gsplat::projection_ewa_3dgs_packed_split_fwd
    );
    m.def(
    ...
    m.def(
        "frustum_culling",
        &gsplat::frustum_culling
    );

    m.def(
        "frustum_culling_split",
        &gsplat::frustum_culling_split
    );

    m.def(
        "rasterize_to_pixels_3dgs_fwd", &gsplat::rasterize_to_pixels_3dgs_fwd
    );m.def(
        "rasterize_to_pixels_3dgs_split_fwd", &gsplat::rasterize_to_pixels_3dgs_split_fwd
    );
    m.def(
        "rasterize_to_pixels_3dgs_bwd", &gsplat::rasterize_to_pixels_3dgs_bwd
    );
    m.def(
        "rasterize_to_pixels_3dgs_split_bwd", &gsplat::rasterize_to_pixels_3dgs_split_bwd
    );
