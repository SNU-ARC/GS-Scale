...
def frustum_culling_split(
    ...
    viewmats = viewmats.contiguous()
    Ks = Ks.contiguous()
    return _FrustumCullingSplit.apply(
        means,
        quats,
        scales,
        viewmats,
...
def fully_fused_projection_split(
    means: Tensor,  # [N, 3]
    covars: Optional[Tensor],  # [N, 6] or None
    quats: Optional[Tensor],  # [N, 4] or None
    scales: Optional[Tensor],  # [N, 3] or None
    ...
    Ks = Ks.contiguous()
    if packed:
        return _FullyFusedProjectionPackedSplit.apply(
            means,
            covars,
            quats,
            scales,
...
def isect_tiles_split(
    ...
    tiles_per_gauss, isect_ids, flatten_ids = _make_lazy_cuda_func("intersect_tile_split")(
        means2d.contiguous(),
        radii.contiguous(),
        depths.contiguous(),
        camera_ids,
...
def rasterize_to_pixels_split(
    ...
    render_colors, render_alphas = _RasterizeToPixelsSplit.apply(
        means2d.contiguous(),
        conics.contiguous(),
        colors.contiguous(),
        opacities.contiguous(),
...
class _FrustumCulling(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means: Tensor,  # [N, 3]
        ...
        valid_ids = _make_lazy_cuda_func(
            "frustum_culling"
        )(
            means,
            quats,
...
class _FrustumCullingSplit(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means: Tensor,  # [N, 3]
...
class _RasterizeToPixelsSplit(torch.autograd.Function):
    """Rasterize gaussians"""
    ...
    def forward(
        ...
        render_colors, render_alphas, last_ids = _make_lazy_cuda_func(
            "rasterize_to_pixels_3dgs_split_fwd"
        )(
            means2d,
            conics,
            colors,
    ...
    def backward(
        ...
        (
            ...
            v_colors,
            v_opacities,
        ) = _make_lazy_cuda_func("rasterize_to_pixels_3dgs_split_bwd")(
            means2d,
            conics,
            colors,
            opacities,
...
class _FullyFusedProjectionPackedSplit(torch.autograd.Function):
    """Projects Gaussians to 2D. Return packed tensors."""
    ...
    def forward(
        ...
        (
            ...
            conics,
            compensations,
        ) = _make_lazy_cuda_func("projection_ewa_3dgs_packed_split_fwd")(
            means,
            covars,  # optional
            quats,  # optional
            scales,  # optional
