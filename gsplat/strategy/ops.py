...
def duplicate(
    ...
    if is_partial:
        device_gpu = params["scales"].device
        sel_gpu = sel.to(device_gpu)

    def param_fn(name: str, p: Tensor) -> Tensor:
        if is_partial and p.device != device:
            param = torch.nn.Parameter(torch.cat([p, p[sel_gpu]]), requires_grad=p.requires_grad)
        else:
            param = torch.nn.Parameter(torch.cat([p, p[sel]]), requires_grad=p.requires_grad)
        return param

    def optimizer_fn(key: str, v: Tensor) -> Tensor:
        if is_partial and v.device != device:
            optim = torch.cat([v, torch.zeros((len(sel_gpu), *v.shape[1:]), device=device_gpu)])
        else:
            optim = torch.cat([v, torch.zeros((len(sel), *v.shape[1:]), device=device)])
        return optim
...
def split(
    ...
    if is_partial:
        device_gpu = params["scales"].device
        sel_gpu = sel.to(device_gpu)
        rest_gpu = rest.to(device_gpu)

    if is_partial:
        scales = torch.exp(params["scales"][sel_gpu])
        quats = F.normalize(params["quats"][sel_gpu], dim=-1)
        rotmats = normalized_quat_to_rotmat(quats)  # [N, 3, 3]
        samples = torch.einsum(
            "nij,nj,bnj->bni",
            rotmats,
            scales,
            torch.randn(2, len(scales), 3, device=device_gpu),
        )  # [2, N, 3]
    else:
    ...
    def param_fn(name: str, p: Tensor) -> Tensor:
        if is_partial:
            repeats = [2] + [1] * (p.dim() - 1)
            if name == "means":
                p_split = (p[sel_gpu] + samples).reshape(-1, 3)  # [2N, 3]
                p_new = torch.cat([p[rest_gpu], p_split])
            elif name == "scales":
                ...
                p_split = p[sel_gpu].repeat(repeats)
                p_new = torch.cat([p[rest_gpu], p_split])
            else:
    ...
    def optimizer_fn(key: str, v: Tensor) -> Tensor:
        if is_partial and v.device != device:
            v_split = torch.zeros((2 * len(sel_gpu), *v.shape[1:]), device=device_gpu)
            optim = torch.cat([v[rest_gpu], v_split])
        else:
            v_split = torch.zeros((2 * len(sel), *v.shape[1:]), device=device)
            optim = torch.cat([v[rest], v_split])
        return optim
...
def remove(
    ...
    def param_fn(name: str, p: Tensor) -> Tensor:
        if is_partial and p.device != device:
            param = torch.nn.Parameter(p[sel_gpu], requires_grad=p.requires_grad)
        else:
            param = torch.nn.Parameter(p[sel], requires_grad=p.requires_grad)
        return param
