...
class DefaultStrategy(Strategy):
    ...
    def step_post_backward_only_update(
        self,
        params: Union[Dict[str, torch.nn.Parameter], torch.nn.ParameterDict],
        state: Dict[str, Any],
        step: int,
    ...
    def step_post_backward_wo_update(
        self,
        params: Union[Dict[str, torch.nn.Parameter], torch.nn.ParameterDict],
        optimizers: Dict[str, torch.optim.Optimizer],
        state: Dict[str, Any],
        ...
        """Callback function to be executed after the `loss.backward()` call."""
        is_partial = False      # Partial offloading
        if params["scales"].device != params["sh0"].device:
            is_partial = True
        is_densify = False
        if step >= self.refine_stop_iter:
            return False
        ...
        if (
            ...
            if is_partial:
                state["grad2d"] = state["grad2d"].cpu()
                state["count"] = state["count"].cpu()
            # grow GSs
            ...
            if is_partial:
                state["grad2d"] = state["grad2d"].to(params["scales"].device)
                state["count"] = state["count"].to(params["scales"].device)
    ...
    def _update_state(
        ...
        if state["grad2d"] is None:
            if is_partial:
                state["grad2d"] = torch.zeros(n_gaussian, device=params["scales"].device)
            else:
                state["grad2d"] = torch.zeros(n_gaussian, device=grads.device)
        if state["count"] is None:
            if is_partial:
                state["count"] = torch.zeros(n_gaussian, device=params["scales"].device)
            else:
        ...
        if is_partial:
            gs_ids_gpu = gs_ids.to(params["scales"].device)
            grads_gpu = grads.to(params["scales"].device)
            state["grad2d"].index_add_(0, gs_ids_gpu, grads_gpu.norm(dim=-1))
            state["count"].index_add_(
                0, gs_ids_gpu, torch.ones_like(gs_ids_gpu, dtype=torch.float32)
            )
        else:
    ...
    def _grow_gs(
        ...
        if is_partial:
            is_small = (
                torch.exp(params["scales"].to("cpu")).max(dim=-1).values
        ...
        # first duplicate
        if n_dupli > 0:
            duplicate(params=params, optimizers=optimizers, state=state, mask=is_dupli, is_partial=is_partial)
    ...
    def _prune_gs(
        ...
        if n_prune > 0:
            remove(params=params, optimizers=optimizers, state=state, mask=is_prune, is_partial=is_partial)
