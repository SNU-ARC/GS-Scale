...
from .cuda._wrapper import (
    frustum_culling,
    frustum_culling_split,
    fully_fused_projection,
    fully_fused_projection_split,
    fully_fused_projection_2dgs,
    isect_offset_encode,
    isect_tiles,
    isect_tiles_split,
    rasterize_to_pixels,
    rasterize_to_pixels_split,
...
def frustum_culling_gpu(
    ...
    valid_ids = frustum_culling(means, quats, scales, viewmats, Ks, width, height, eps2d, near_plane, far_plane) 
    return valid_ids

def frustum_culling_gpu_split(
    means: Tensor,  # [N, 3]
    quats: Tensor,  # [N, 4]
    scales: Tensor,  # [N, 3]
    viewmats: Tensor,  # [C, 4, 4]
    ...
    valid_ids = frustum_culling_split(means, quats, scales, viewmats, Ks, width, height, eps2d, near_plane, far_plane, split_point, left) 
    return valid_ids
...
def rasterization(
    ...
    width: int,
    height: int,
    colors0: Tensor = None,  # [(C,) N, D] or [(C,) N, K, 3]
    near_plane: float = 0.01,
    far_plane: float = 1e10,
    ...
    if sh_degree is None:
        ...
        if packed:
            ...
            if colors.dim() == 3:
                ...
                shs = torch.cat([colors0[gaussian_ids], colors[gaussian_ids]], 1)  # [nnz, K, 3]
            else:
                # Turn [C, N, K, 3] into [nnz, 3]
                shs = torch.cat([colors0[camera_ids, gaussian_ids], colors[camera_ids, gaussian_ids]], 1)  # [nnz, K, 3]
            ...
            if colors.dim() == 3:
                ...
                colors = torch.cat([colors0, colors], 1)
                shs = colors.expand(C, -1, -1, -1)  # [C, N, K, 3]
            else:
                # colors is already [C, N, K, 3]
                colors = torch.cat([colors0, colors], 2)
                shs = colors
...
def rasterization_split(
    ...
    # Project Gaussians to 2D. Directly pass in {quats, scales} is faster than precomputing covars.
    proj_results = fully_fused_projection_split(
        means,
        covars,
        quats,
        scales,
    ...
    tile_width = math.ceil(width / float(tile_size))
    tile_height = math.ceil(height / float(tile_size))
    tiles_per_gauss, isect_ids, flatten_ids = isect_tiles_split(
        means2d,
        radii,
        depths,
        tile_size,
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            render_colors_, render_alphas_ = rasterize_to_pixels_split(
                means2d,
                conics,
                colors_chunk,
                opacities,
        ...
        render_colors, render_alphas = rasterize_to_pixels_split(
            means2d,
            conics,
            colors,
            opacities,
