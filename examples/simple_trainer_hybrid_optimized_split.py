...
from gsplat.rendering import rasterization, rasterization_split
from gsplat.rendering import frustum_culling_gpu, frustum_culling_gpu_split
...
class Runner:
    ...
    def rasterize_splats_only_split(
        self,
        camtoworlds: Tensor,
        Ks: Tensor,
        means: Tensor,
        ...
        **kwargs,
        ...
        rasterize_mode = "antialiased" if self.cfg.antialiased else "classic"
        render_colors, render_alphas, info = rasterization_split(
            means=means,
            quats=quats,
            scales=scales,
            opacities=opacities,
    ...
    def precompute_split_points(self, device):
        print("Pre-computing optimal split points for all images...")
        split_points = {}
        ...
        for data in tqdm.tqdm(precompute_loader, desc="Finding optimal splits"):
            ...
            for step in range(5):
                ...
                mid_point = round(mid_point / 16) * 16

                valid_ids_left = frustum_culling_gpu_split(
                    means, quats, scales, 
                    torch.linalg.inv(camtoworlds), Ks, 
                    width, height, 
                    split_point=mid_point, left=True
                )
                valid_ids_right = frustum_culling_gpu_split(
                    means, quats, scales, 
                    torch.linalg.inv(camtoworlds), Ks, 
                    width, height, 
                    split_point=mid_point, left=False
                )

                left_count = len(valid_ids_left)
                right_count = len(valid_ids_right)
            ...
            if best_split < 32:
                best_split = 32
            if best_split > width - 32:
                best_split = width / 16 * 16 - 32
            split_points[image_id] = best_split
            # tqdm.tqdm.write(f"Image {image_id}: split={best_split}, balance={best_balance:.3f}")
        
        return split_points

    def gpu_forward_backward(self, step, camtoworlds, Ks, valid_ids, opacities, colors0, colorsN, width, height, image_ids, masks, pixels, results, valid_ids_left, valid_ids_right, valid_ids_left_index, valid_ids_right_index):
        cfg = self.cfg
        device = self.device
        # sh schedule
        sh_degree_to_use = min(step // cfg.sh_degree_interval, cfg.sh_degree)
        
        if valid_ids.shape[0] / self.splats["means"].shape[0] > cfg.split_threshold:
            image_id = image_ids.item()
            split_point = self.split_points[image_id]

            # Left rendering
            split_left = split_point + 16

            renders, alphas, info = self.rasterize_splats_only_split(
                camtoworlds=camtoworlds,
                Ks=Ks,
                means=self.splats["means"][valid_ids_left],
                quats=self.splats["quats"][valid_ids_left],
                scales=torch.exp(self.splats["scales"][valid_ids_left]),
                opacities=opacities[valid_ids_left_index],
                colors0=colors0[valid_ids_left_index],
                colorsN=colorsN[valid_ids_left_index],
                width=width,
                height=height,
                left=True,
                split_point=split_left,
            ...
            pixels_l1 = pixels[:, :, :split_point, :]
            pixels_ssim = pixels[:, :, :split_point + 5, :]

            colors_grad = colors[:, :, :split_point, :]
            colors_ssim = colors[:, :, :split_point + 5, :]

            # loss
            l1loss = F.l1_loss(colors_grad, pixels_l1)
            l1loss_scale = l1loss / width * split_point
            ssimloss = 1.0 - fused_ssim(
                colors_ssim.permute(0, 3, 1, 2), pixels_ssim.permute(0, 3, 1, 2), padding="valid"
            )
            ssimloss_scale = ssimloss / (width - 10) * (split_point - 5)
            loss_left = l1loss_scale * (1.0 - cfg.ssim_lambda) + ssimloss_scale * cfg.ssim_lambda

            # Move info from gpu to cpu for densification
            info["gaussian_ids"] = valid_ids_left[info["gaussian_ids"].to("cpu", non_blocking=False)]
            ...
            l1loss_split = l1loss_scale
            ssimloss_split = ssimloss_scale

            # Manually backpropagate grads from gpu to cpu. Backprop for means, quats, and scales are done via autograd.
            left_grads = {}

            opacities_grad = opacities.grad[valid_ids_left_index] * opacities.data[valid_ids_left_index] * (1 - opacities.data[valid_ids_left_index])
            opacities_cpu_grad = torch.empty_like(opacities_grad, device="cpu", pin_memory=True)
            opacities_cpu_grad.copy_(opacities_grad, non_blocking=False)
            left_grads["opacities"] = opacities_cpu_grad
            
            colors0_cpu_grad = torch.empty_like(colors0.grad[valid_ids_left_index], device="cpu", pin_memory=True)
            colors0_cpu_grad.copy_(colors0.grad[valid_ids_left_index], non_blocking=False)
            left_grads["sh0"] = colors0_cpu_grad
            
            colorsN_cpu_grad = torch.empty_like(colorsN.grad[valid_ids_left_index], device="cpu", pin_memory=True)
            colorsN_cpu_grad.copy_(colorsN.grad[valid_ids_left_index], non_blocking=False)
            left_grads["shN"] = colorsN_cpu_grad
            ...
            # Preserve data for info
            left_gaussian_ids = info["gaussian_ids"]
            radii_left = info["radii"] 
            means2d_data_left = info["means2d"].data
            means2d_grad_left = info["means2d"].grad

            # Right rendering
            split_right = split_point - 16
            
            renders, alphas, info = self.rasterize_splats_only_split(
                camtoworlds=camtoworlds,
                Ks=Ks,
                means=self.splats["means"][valid_ids_right],
                quats=self.splats["quats"][valid_ids_right],
                scales=torch.exp(self.splats["scales"][valid_ids_right]),
                opacities=opacities[valid_ids_right_index],
                colors0=colors0[valid_ids_right_index],
                colorsN=colorsN[valid_ids_right_index],
                width=width,
                height=height,
                left=False,
                split_point=split_right,
                sh_degree=sh_degree_to_use,
                near_plane=cfg.near_plane,
                far_plane=cfg.far_plane,
                image_ids=image_ids,
            ...
            colors_grad = colors[:, :, 16:, :]
            colors_ssim = colors[:, :, 11:, :]

            # loss
            l1loss = F.l1_loss(colors_grad, pixels_l1)
            l1loss_scale = l1loss / width * (width - split_point)
            ssimloss = 1.0 - fused_ssim(
                colors_ssim.permute(0, 3, 1, 2), pixels_ssim.permute(0, 3, 1, 2), padding="valid"
            )
            ssimloss_scale = ssimloss / (width - 10) * ((width - split_point) - 5)
            loss_right = l1loss_scale * (1.0 - cfg.ssim_lambda) + ssimloss_scale * cfg.ssim_lambda

            # Move info from gpu to cpu for densification
            info["gaussian_ids"] = valid_ids_right[info["gaussian_ids"].to("cpu", non_blocking=False)]
            info["radii"] = info["radii"].to("cpu", non_blocking=False)
            ...
            opacities_grad = opacities.grad[valid_ids_right_index] * opacities.data[valid_ids_right_index] * (1 - opacities.data[valid_ids_right_index])
            opacities_cpu_grad = torch.empty_like(opacities_grad, device="cpu", pin_memory=True)
            opacities_cpu_grad.copy_(opacities_grad, non_blocking=False)
            right_grads["opacities"] = opacities_cpu_grad
            
            colors0_cpu_grad = torch.empty_like(colors0.grad[valid_ids_right_index], device="cpu", pin_memory=True)
            colors0_cpu_grad.copy_(colors0.grad[valid_ids_right_index], non_blocking=False)
            right_grads["sh0"] = colors0_cpu_grad
            
            colorsN_cpu_grad = torch.empty_like(colorsN.grad[valid_ids_right_index], device="cpu", pin_memory=True)
            colorsN_cpu_grad.copy_(colorsN.grad[valid_ids_right_index], non_blocking=False)
            right_grads["shN"] = colorsN_cpu_grad
            ...
            # Preserve data for info
            right_gaussian_ids = info["gaussian_ids"]
            radii_right = info["radii"] 
            means2d_data_right = info["means2d"].data
            means2d_grad_right = info["means2d"].grad

            # CPU Indexing Version
            # Generate info
            info = {}
            
            gaussian_ids = torch.cat([left_gaussian_ids, right_gaussian_ids])
            gaussian_ids = torch.unique(gaussian_ids, sorted=True)

            left_indices = torch.searchsorted(gaussian_ids, left_gaussian_ids)
            right_indices = torch.searchsorted(gaussian_ids, right_gaussian_ids)

            num_total = gaussian_ids.shape[0]

            info["gaussian_ids"] = gaussian_ids
            info["width"] = width
            info["height"] = height
            info["n_cameras"] = 1

            results["grads"] = {
                "left_index": valid_ids_left,
                "left_grads": left_grads,
                "right_index" : valid_ids_right,
                "right_grads": right_grads,
            }
            ...
            results["info_split"] = {
                ...
                "radii_left": radii_left,
                "means2d_data_left": means2d_data_left,
                "means2d_grad_left": means2d_grad_left,
                "right_indices": right_indices,
                "radii_right": radii_right,
                "means2d_data_right": means2d_data_right,
                "means2d_grad_right": means2d_grad_right,
            }

            l1loss = l1loss_split
            ssimloss = ssimloss_split
        else:
            # forward
            ...
            # Device to Host
            # Manually backpropagate grads from gpu to cpu. Backprop for means, quats, and scales are done via autograd.
            opacities_cpu_grad = torch.empty_like(opacities.data, device="cpu", pin_memory=True)
            opacities_grad = opacities.grad * opacities.data * (1 - opacities.data)     # derivative of sigmoid
            opacities_cpu_grad.copy_(opacities_grad, non_blocking=False) 
    
            colors0_cpu_grad = torch.empty_like(colors0.data, device="cpu", pin_memory=True)
            colors0_grad = colors0.grad
            colors0_cpu_grad.copy_(colors0_grad, non_blocking=False)
    
            colorsN_cpu_grad = torch.empty_like(colorsN.data, device="cpu", pin_memory=True)
            colorsN_grad = colorsN.grad
            ...
            results["grads"] = {
                "opacities_cpu_grad": opacities_cpu_grad,
                "colors0_cpu_grad": colors0_cpu_grad,
                "colorsN_cpu_grad": colorsN_cpu_grad,
            }
    ...
    def prologue(self, trainloader_iter, schedulers):
        ...
        if valid_ids.shape[0] / self.splats["means"].shape[0] > cfg.split_threshold:
            ...
            valid_ids_left = frustum_culling_gpu_split(means, quats, scales, torch.linalg.inv(camtoworlds), Ks, width, height, split_point = split_point, left = True)
            valid_ids_right = frustum_culling_gpu_split(means, quats, scales, torch.linalg.inv(camtoworlds), Ks, width, height, split_point = split_point, left = False)

            valid_ids_left_index = torch.searchsorted(valid_ids, valid_ids_left)
            valid_ids_right_index = torch.searchsorted(valid_ids, valid_ids_right)
        else:
            valid_ids_left = None
            valid_ids_right = None
            valid_ids_left_index = None
            valid_ids_right_index = None
        ...
        return is_densify, valid_ids, valid_ids_cpu, valid_ids_cpu_prev, data_list, param_list, valid_ids_left, valid_ids_right, valid_ids_left_index, valid_ids_right_index
    ...
    def train(self):
        ...
        self.split_points = self.precompute_split_points(device)
        ...
        is_densify, valid_ids, valid_ids_cpu, valid_ids_cpu_prev, data_list, param_list, valid_ids_left, valid_ids_right, valid_ids_left_index, valid_ids_right_index = self.prologue(trainloader_iter, schedulers)
...
#        with profile(schedule=torch.profiler.schedule(wait=80, warmup=10, active=10), on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/rubble_hybrid_opt3')) as p:
        for step in pbar:
            ...
            subthread = threading.Thread(
                ...
                args=(step, camtoworlds, Ks, valid_ids, opacities, colors0, colorsN, width, height, image_ids, masks, pixels, results, valid_ids_left, valid_ids_right, valid_ids_left_index, valid_ids_right_index, ), 
                daemon=False
            )
            subthread.start()
            ...
            info = results["info"]
            if results["split"]:
                info_split = results["info_split"]

                num_total = info_split["num_total"]
                
                combined_radii = torch.zeros(num_total, 2, device="cpu", dtype=torch.int32, pin_memory=True)
                combined_means2d_data = torch.zeros(num_total, 2, device="cpu", dtype=torch.float32, pin_memory=True)
                combined_means2d_grad = torch.zeros_like(combined_means2d_data, pin_memory=True)

                left_indices_cpu = info_split["left_indices"].to("cpu", non_blocking=False)
                right_indices_cpu = info_split["right_indices"].to("cpu", non_blocking=False)

                combined_radii[left_indices_cpu] = info_split["radii_left"]
                combined_means2d_data[left_indices_cpu] = info_split["means2d_data_left"]
                combined_means2d_grad[left_indices_cpu] = info_split["means2d_grad_left"]

                combined_radii[right_indices_cpu] = info_split["radii_right"]
                combined_means2d_data[right_indices_cpu] = info_split["means2d_data_right"]
                combined_means2d_grad[right_indices_cpu] += info_split["means2d_grad_right"]

                info["radii"] = combined_radii
                info["means2d"] = Variable(combined_means2d_data, requires_grad=True)
                info["means2d"].grad = combined_means2d_grad
            ...
            grads_data = results["grads"]

            if results["split"]:
                # Split
                left_idx = grads_data["left_index"].to("cpu", non_blocking=False)
                right_idx = grads_data["right_index"].to("cpu", non_blocking=False)

                left_grads = grads_data["left_grads"]
                right_grads = grads_data["right_grads"]

                for attr in ["opacities", "sh0", "shN"]:
                    self.splats[attr].grad.index_copy_(0, left_idx, left_grads[attr])
                    self.splats[attr].grad.index_add_(0, right_idx, right_grads[attr])
            else:
                opacities_cpu_grad = grads_data["opacities_cpu_grad"]
                colors0_cpu_grad = grads_data["colors0_cpu_grad"] 
                colorsN_cpu_grad = grads_data["colorsN_cpu_grad"]
